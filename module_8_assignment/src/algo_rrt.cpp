...
RRT_Planner::RRT_Planner() {
    DOMAIN.fill(0); // Initialize DOMAIN to 0
}
...
bool RRT_Planner::isGoalFound(NodeRrt& newNode, NodeRrt const& goal) {
    float distance = NodeRrt::heuristicsEuclid(newNode, goal);
    return distance <= GOAL_RADIUS; // Use GOAL_RADIUS for tolerance
}
...
bool RRT_Planner::isObstacle(NodeRrt const& nearestNode, NodeRrt const& newNode) {
    ...
    // Check for direct obstacles
    if (DOMAIN[newNode.getY() * GRID_WIDTH + newNode.getX()] == 100) {
        return true;
    }

    // Perform line interpolation for collisions
